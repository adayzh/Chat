    ...
    CustomDialog(parent)
{
    ...
    labelWinIcon->setPixmap(QPixmap(":/resource/images/app.png"));
...
void CMessageBox::ShowMessage(const QString &content, const quint8 &msgType, const QString &title)
{
    ...
    btnCancel->setVisible(CMessageBox::Questions == msgType);

    if (CMessageBox::Information == msgType)
        labelIcon->setPixmap(QPixmap(":/resource/images/ic_info.png"));
    else if (CMessageBox::Warnings == msgType)
        labelIcon->setPixmap(QPixmap(":/resource/images/ic_warning.png"));
    else if (CMessageBox::Questions == msgType)
...
int CMessageBox::Infomation(QWidget *parent, const QString &content, const QString &title)
{
    ...
    messageBox->ShowMessage(content, CMessageBox::Information, title);
...
{
    CMessageBox *messageBox = new CMessageBox(parent);
    messageBox->ShowMessage(content, CMessageBox::Questions, title);
    return messageBox->exec();
}
...
{
    CMessageBox *messageBox = new CMessageBox(parent);
    messageBox->ShowMessage(content, CMessageBox::Warnings, title);
    return messageBox->exec();
}
